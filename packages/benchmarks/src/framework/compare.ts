...
export async function compare(
  name: string,
  fns: { [name: string]: () => void | Promise<void> },
  opts?: CompareOptions
): Promise<void> {
  ...
  const lenOfLongestFnName = Object.keys(fns).reduce((a, b) =>
  ...
  for (const name of fnKeys) {
    ...
    console.log(
      `  ${paddedName}   took ${avg.toFixed(5)}ms, high ${high.toFixed(
        5
      )}ms, low ${low.toFixed(5)}ms`
    );
  }
}
