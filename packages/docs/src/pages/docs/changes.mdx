...
```ts
const watcher = await watch(userTable, { where: { age: { lt: 3 } } }, babies => {
  console.log('All babies: ', babies);
});
```

If you want to stop receiving updates, call the `.close()` function on the object returned from [`watch()`](/docs/reference/watch).
