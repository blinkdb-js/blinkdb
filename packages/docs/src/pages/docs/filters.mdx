...
Retrieving ALL users from the database can be a huge undertaking, especially
...
- `skip`, or at which index to begin
...
Offset-based pagination is inferior to cursor-based pagination,
...
const users = await many(userTable, {
  limit: {
    from: lastUserId,
    // In order to not return the lastUser of the previous page
    skip: 1,
    take: 10
  }
});
