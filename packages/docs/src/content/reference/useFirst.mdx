...
`useFirst()` is asynchronous, which means the queried data isn't available immediately.
...
const Component = () => {
  const userTable = useTable((model: Model) => model.users);
  const { data: firstUser, state, error } = useFirst(userTable, {
    where: {
      name: "Alice"
    }
  });
...
const { data: alice } = useFirst(userTable, "alice-uuid");
```
