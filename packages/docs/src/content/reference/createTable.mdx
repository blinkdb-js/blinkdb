...
| --------- | -------------------------------------------------- |
| `db`      | The database created by [`createDB()`](createdb) . |
...
At runtime, you can check that you can use an object in a table with [`isValidEntity`](isvalidentity).
...
[`first()`](first) / [`one()`](one) / [`many()`](many) / [`watch()`](watch) -
