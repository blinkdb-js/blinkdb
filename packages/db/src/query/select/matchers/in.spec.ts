...
it("should return all items by key if there are multiple items in in", () => {
  expect(collect<Data>((fn) => selectForIn(btree, { in: ["0", "1"] }, fn))).toStrictEqual(
    [{ a: "Hello" }, { a: "There" }]
  );
  expect(collect<Data>((fn) => selectForIn(btree, { in: ["0", "2"] }, fn))).toStrictEqual(
    [{ a: "Hello" }, { a: "World" }]
  );
});
...
it("should ignore the order of keys in in", () => {
  ...
  expect(collect<Data>((fn) => selectForIn(btree, { in: ["2", "0"] }, fn))).toStrictEqual(
    [{ a: "Hello" }, { a: "World" }]
  );
});
...
it("should return nothing for unknown keys", () => {
  ...
  expect(collect<Data>((fn) => selectForIn(btree, { in: ["a", "b"] }, fn))).toStrictEqual(
    []
  );
  expect(
