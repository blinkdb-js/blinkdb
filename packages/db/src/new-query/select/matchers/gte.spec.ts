...
it("should return nothing if the btree is empty", () => {
  expect(collect<Data>(fn => selectForGte<Data, "a">(new BTree(), { $gte: "1" }, fn))).toStrictEqual([]);
});

it("should return all matching items if found", () => {
  expect(collect<Data>(fn => selectForGte<Data, "a">(btree, { $gte: "1" }, fn))).toStrictEqual([
    { a: "There" },
    { a: "World" }
  ]);
});
...
it("should return nothing if the given key is greater than the biggest existing key", () => {
  expect(collect<Data>(fn => selectForGte<Data, "a">(btree, { $gte: "z" }, fn))).toStrictEqual([]);
});

it("should return everything if the given key is lesser than the smallest existing key", () => {
  expect(collect<Data>(fn => selectForGte<Data, "a">(btree, { $gte: "" }, fn))).toStrictEqual(btree.valuesArray());
});