...
it("should return nothing if the btree is empty", () => {
  expect(collect<Data>(fn => selectForGt<Data, "a">(new BTree(), { $gt: "1" }, fn))).toStrictEqual([]);
});

it("should return all matching items if found", () => {
  expect(collect<Data>(fn => selectForGt<Data, "a">(btree, { $gt: "1" }, fn))).toStrictEqual([
    { a: "World" }
  ]);
});

it("should return nothing if the given key is greater than the biggest existing key", () => {
  expect(collect<Data>(fn => selectForGt<Data, "a">(btree, { $gt: "z" }, fn))).toStrictEqual([]);
});

it("should return everything if the given key is lesser than the smallest existing key", () => {
  expect(collect<Data>(fn => selectForGt<Data, "a">(btree, { $gt: "" }, fn))).toStrictEqual(btree.valuesArray());
});