...
it("should return nothing if the btree is empty", () => {
  expect(collect<Data>(fn => selectForLte<Data, "a">(new BTree(), { $lte: "1" }, fn))).toStrictEqual([]);
});

it("should return all matching items if found", () => {
  expect(collect<Data>(fn => selectForLte<Data, "a">(btree, { $lte: "1" }, fn))).toStrictEqual([
    { a: "Hello" },
    { a: "There" }
  ]);
});
...
it("should return everything if the given key is greater than the biggest existing key", () => {
  expect(collect<Data>(fn => selectForLte<Data, "a">(btree, { $lte: "z" }, fn))).toStrictEqual(btree.valuesArray());
});

it("should return nothing if the given key is lesser than the smallest existing key", () => {
  expect(collect<Data>(fn => selectForLte<Data, "a">(btree, { $lte: "" }, fn))).toStrictEqual([]);
});